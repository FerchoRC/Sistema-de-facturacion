...
const AllInvoices = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Estas seguro de eliminar esta factura?')){
      try {
        ...
        setError('Error eliminando la factura');
        console.error(err)
      }
    }
  };
  ...
  const handleStatusChange = async (invoice) => {
    ...
    try {
      ...
      setError('Error actualizando el estatus de la factura');
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Gestiona todas tus facturas en un solo sitio
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
        <div className="p-4 sm:p-6 border border-b border-slate-200">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-grow">
              ...
              <input
                type="text"
                placeholder="Buscar por el # de factura"
        ...
        {filteredInvoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-2">No se encontraron facturas</h3>
            <p className="text-slate-500 mb-6 max-w-md">Prueba otros criterios de busqueda</p>
            {invoices.length === 0 && (
              <Button onClick={() => navigate('/invoices/new')} icon={Plus}>Crear factura</Button>
            )}
          </div>
        ) : (
          <div className="w-[90vw] md:w-auto overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Factura</th>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Opciones</th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                        <Button size="small" variant="secondary" onClick={() => handleStatusChange(invoice)} isLoading={statusChangeLoading === invoice._id}>
                          {invoice.status === 'Pagado' ? 'Marcar Pendiente' : 'Marcar Pagado'}
                        </Button>
