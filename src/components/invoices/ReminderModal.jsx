...
const ReminderModal = ({isOpen, onClose, invoiceId}) => {
    ...
    if (isOpen && invoiceId){
      const generateReminder = async () => {
        ...
        try {
          ...
          console.error('Error con el recordatorio', error);
          onClose();
        } finally {
          setIsLoading(false)
        }
  ...
  const handleCopyToClipboard = () => {
    ...
    setHasCopied(true);
    toast.success('Recordatorio copiado en el porta papeles!');
      ...
      <div className="flex items-center justify-center min-h-screen px-4 text-center">
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 relative text-left transform transition-all">
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <Button onClick={handleCopyToClipboard} icon={hasCopied ? Check : Copy} disabled={isLoading}>{hasCopied ? 'Copiado' : 'Copiar texto'}</Button>
          </div>
        </div>
      </div>
    </div>
